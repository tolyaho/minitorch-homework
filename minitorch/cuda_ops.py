...
def device_jit(fn: Fn, **kwargs: Any) -> Fn:
    """Compile a function for the CUDA device."""
...
class CudaOps(TensorOps):
    ...
    def matrix_multiply(a: Tensor, b: Tensor) -> Tensor:
        """Batched CUDA tensor matrix multiply."""
...
def sum_practice(a: Tensor) -> TensorData:
    """Run the CUDA sum practice kernel."""
    (size,) = a.shape
    threadsperblock = THREADS_PER_BLOCK
...
def tensor_reduce(
    ...
    def _reduce(
        ...
        o = index_to_position(out_index, out_strides)
        reduce_i = out_index[reduce_dim] * BLOCK_DIM + pos
        out_index[reduce_dim] = reduce_i
        if reduce_i < a_shape[reduce_dim]:
...
def mm_practice(a: Tensor, b: Tensor) -> TensorData:
    """Run the CUDA MM practice kernel."""
    (size, _) = a.shape
